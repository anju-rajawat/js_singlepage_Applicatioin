...
function Createheder(){
    ...
    header.setAttribute("style","width: 100%; height: 70px; background-color: black;");
    // search input
    var div=document.createElement("div");
    div.setAttribute("style","width: 30%; height: 33px; border:1px solid black; position: absolute; top:20px;left:30%;border-radius: 15px;");
    
    var searchInput = document.createElement("input");
    searchInput.setAttribute("type","text");
    searchInput.setAttribute("placeholder","Search product");
    searchInput.setAttribute("style","width:100%;height:40px; border-radius: 25px;text-align: center;");
   ...
   div.appendChild(searchInput);
    ...
    menuDiv.setAttribute("style","width:25%;height:40px; position: absolute; top:15px;left:70%; display:flex;justify-content:space-around;align-items:center;");
   
    if(isLoggedIn()){
        var viewCard=document.createElement("span");
        viewCard.innerText="View Cart";
        viewCard.setAttribute("style","color:white;cursor:pointer");
        viewCard.addEventListener("click",()=>{
        var carDiv=document.querySelector("#cart-container");
        carDiv.innerHTML="";
        ...
        hr.setAttribute("class","text-center");
    ...
    sign.setAttribute("style","color:white; cursor:pointer;");
   ...
   signup.setAttribute("style","color:white");
   // new div start
...
function createCard(data){
    ...
    for(let product of data){
       ...
       cartDiv.setAttribute("style","width:30%; height:500px;box-shadow: 10px 10px 10px grey;");       
       ...
       addToCart.setAttribute("style","width:90%; height:40px; margin-left:20px")
...
function SignInComponent(){
    var mainDiv = document.querySelector("#main");   
    mainDiv.innerHTML = "";
    //mainDiv.setAttribute("style","background-image: url(1.jpeg);");

    var firstcon=document.createElement("div");
    firstcon.setAttribute("style","background-image: url(Whats.jpg); height:800px; width:100%; background-repeat: no-repeat; background-size: cover;");

    var containersign=document.createElement("div");
    containersign.setAttribute("style","background:transparent; width:30%; height: 300px;position:absolute; top:50%; left:50%; transform:translate(-50%,-50%);box-shadow: 10px 10px 10px 10px grey; background-color: rgb(218, 118, 168);");

    
    main.appendChild(firstcon);
    firstcon.appendChild(containersign);
    
    
    var password=document.createElement("input");

    password.setAttribute("placeholder","Enter password");
    password.setAttribute("style","width:90%; height: 40px; margin:15px;border-radius: 20px;")
    password.setAttribute("id","password");
    
    var Email=document.createElement("input");
    Email.setAttribute("placeholder","Enter Email");
    Email.setAttribute("style","width:90%; height: 40px;margin: 15px;border-radius: 20px;")
    
    Email.setAttribute("id","email");
    
    var signbtn=document.createElement("button");
    signbtn.setAttribute("class","btn btn-primary mt-2 ");
    signbtn.setAttribute("style","width:90%; height: 40px;margin: 15px;border-radius: 20px;");
   ...
   signUpLabel.addEventListener("click",()=>{
    ...
    containersign.appendChild(Email);
    containersign.appendChild(password);
    containersign.appendChild(signbtn);
    containersign.appendChild(signUpLabel);
...
function SignUpComponent(){
    ...
    var contenersingup=document.createElement("div");
    contenersingup.setAttribute("style","background:transparent; width:30%; height: 300px;position:absolute; top:50%; left:50%; transform:translate(-50%,-50%);box-shadow: 10px 10px 10px 10px grey;")
    

    
    maindiv.appendChild(firstcon);
    firstcon.appendChild(contenersingup);
    ...
    var password=document.createElement("input");
    password.setAttribute("placeholder","Enter passwor");
    password.setAttribute("style","width:90%; height: 40px; margin:15px;border-radius: 20px;")
   
    password.setAttribute("id","password");

    var sigupbtn=document.createElement("button");

    sigupbtn.setAttribute("style","width:90%; height: 40px;margin: 15px;border-radius: 20px;");
    sigupbtn.setAttribute("class","btn btn-primary mt-2 ");
   

    sigupbtn.innerText="Sign Up";
   
    sigupbtn.addEventListener("click",()=>{
     var email=document.querySelector("#email").value;
     var password=document.querySelector("#password").value;
      if(userExcet(email,password)){
   ...
   signInLabel.addEventListener("click",()=>{
    ...
    contenersingup.appendChild(email);
    contenersingup.appendChild(password);
    contenersingup.appendChild(sigupbtn);
    contenersingup.appendChild(signInLabel);
...
function userExcet(email,password){
...
function signIn(email,password){
    ...
    if(user){
        ...
        Createheder();
        createCard(JSON.parse(localStorage.getItem("productList")));
    }
    else
...
function SignOut(menuDiv){
    ...
    menuDiv.innerHTML="";
    var signoption=document.createElement("span");
    signoption.innerText="Sign In";
    signoption.setAttribute("style","color:white; cursor:pointer;");
    signoption.addEventListener("click",()=>{
        SignInComponent();
    ...
    menuDiv.appendChild(signoption);
    menuDiv.appendChild(signup);
}
// function

function isLoggedIn(){
    return !!sessionStorage.getItem("isLoggedIn"); //true //false/null
}
